  ...
  ui(new Ui::PWM)
{
  ...
  connect(ramp_tim,SIGNAL(timeout()),this,SLOT(tim()));
}
...
PWM::WorkState PWM::getStatus() const
{
  return status;
}
...
void PWM::RampInit()
{
  ...
  disconnect(ui->sFreq,SIGNAL(valueChanged(int)), this, SLOT(guiChange(int)));
}
...
void PWM::RampDeInit()
{
  ...
  ramp_tim->stop();
  connect(ui->sFreq,SIGNAL(valueChanged(int)), this, SLOT(guiChange(int)));
}
